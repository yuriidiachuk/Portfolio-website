...
const Nav = () => {
  ...
  return (
    ...
    >
      <div
        className="flex w-full lg:flex-col items-center justify-between 
      lg:justify-center gap-y-10 px-4 md:px-40 lg:px-0 h-[80px] lg:h-max py-8 bg-white/10
        ...
        {navData.map((link, index) => {
          return (
            <Link
              ...
              } relative flex items-center group duration-300 hover:scale-150`}
              href={link.path}
              key={index}
            >
              <div className="absolute right-0 hidden pr-14 lg:group-hover:flex">
                <div className="bg-white relative flex text-primary items-center p-[6px]  rounded-[3px]">
                  <div className="text-[12px] leading-none font-semibold capitaliza align-center ">
                    {link.name}
                  </div>
                  <div
